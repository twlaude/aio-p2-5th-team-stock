...
function composePersonal(company: string, topic: string, score: number, level: EvidenceLevel, user: DemoUser): PersonalizedCheckpoints {
  ...
  const opinion: Record<GapState, Record<DemoUser["risk_profile"], string>> = {
    large: {
      conservative: `조금 더 지켜볼까요? ${C} ${topic} 기대만 있고, 공식적으로 확인된 건 아직 부족해요. ${you}이라면 불확실성이 큰 상태예요.`,
      balanced: `급하게 판단하지 않아도 돼요. ${C} ${topic} 기대가 앞서 있는 상황이에요. ${you}이라면 분위기에 휩쓸리기보다 공시로 확인하면서 천천히 봐도 늦지 않아요.`,
      aggressive: `변동 폭이 클 수 있어요. ${C} 지금 ${topic} 기대만으로 관심이 쏠린 상태예요. ${you}이라도 나만의 기준이 없으면 흔들리기 쉬워요.`,
    },
    some: {
      conservative: `공식 발표를 기다려 보세요. ${C} 관심은 뜨겁고 ${T("은", "는")} 절반쯤 확인됐어요. ${you}이라면 다음 실적이나 공시가 나온 뒤 봐도 늦지 않아요.`,
      balanced: `확인되는 만큼만 따라가 보세요. ${C} ${topic} 중 절반은 공식 자료로 확인됐고 나머지는 아직 기대예요. ${you}이라면 한 번에 판단하기보다 나눠서 살펴보는 게 좋아요.`,
      aggressive: `공식 자료가 어느 정도 모였어요. ${C} ${T("이", "가")} 절반은 확인됐어요. 다만 ${you}이라도 기대가 식으면 흐름이 빠르게 바뀔 수 있다는 건 기억해 두세요.`,
    },
    small: {
      conservative: `관심과 근거가 균형을 이루고 있어요. ${C} 시장의 관심과 확인된 재료가 비슷해요. ${you}이라면 ${topic} 실제 흐름만 꾸준히 따라가면 돼요.`,
      balanced: `평소 기준을 지키면 돼요. ${C} 지금 눈에 띄게 앞서가는 신호가 없어요. ${you}이라면 ${T("을", "를")} 중심으로 차분히 살펴보세요.`,
      aggressive: `서두르지 말고 지켜보세요. ${C} ${topic} 대비 관심이 과하지 않아요. ${you}이라면 새로운 소식이 나오는지 차분히 기다려 보세요.`,
    },
    quiet: {
      conservative: `조용하지만 근거는 탄탄해요. ${C} 관심은 적지만 ${T("이", "가")} 공식 자료로 잘 갖춰져 있어요. ${you}이라면 서두르지 않고 차분히 뜯어보기 좋아요.`,
      balanced: `차분하게 살펴보기 좋은 상태예요. ${C} 관심이 적어 들뜬 분위기가 없고 ${T("은", "는")} 확인돼 있어요. ${you}이라면 지금 천천히 재료를 점검해 보기 좋아요.`,
      aggressive: `흐름이 느릴 수 있어요. ${C} 시장의 관심이 낮아서 눈에 띄는 변화가 적을 수 있어요. ${you}이라면 새로운 소식이 나올 때까지 좀 지루할 수 있어요.`,
    },
  };
