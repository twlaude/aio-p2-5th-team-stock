...
async def login(username: str, password: str) -> AuthResponse:
    record = await user_repository.get_by_username(username)
...
async def signup(username: str, password: str, display_name: str, profile: InvestmentProfile) -> AuthResponse:
    try:
        record = await user_repository.create_user(username, password, display_name, profile)
...
async def get_current_user(credentials: HTTPAuthorizationCredentials = Depends(_bearer_required)) -> CurrentUser:
