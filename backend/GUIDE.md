...
Backend는 Frontend의 유일한 서버 진입점이다. 사용자 구분, 투자 성향과 Memory 관리, 지원 종목 확인, MCP Client 요청 및 최종 응답 조립을 담당한다. 개인화 문장은 MCP Client가 생성한다.
...
- Redis 단기 상태 조회
- 관련 Memory에서 허용된 투자 성향 네 값만 선택
- MCP Client에 공통 분석과 개인화 확인 항목 요청
...
│  ├─ schemas/             # 요청·응답 형식
│  ├─ services/            # 인증·Memory·투자 성향·분석 요청 업무 로직
...
Frontend 요청
  ...
  → 기업명 또는 종목 코드 검증
  → 투자 성향과 관련 Memory 조회
  → 정식 기업 정보와 투자 성향 네 값을 MCP Client에 전달
  → 공통 분석과 개인화 확인 포인트 수신
  → 최종 응답 저장·반환
```
