...
- 시세와 수급을 어떤 제공처에서 받을 수 있는가? <뉴스 스크래핑>
- 뉴스 본문을 저장할 수 있는가, 제목·요약만 사용할 것인가? < O>
- 커뮤니티 데이터 확보가 가능한가? < O>
- MCP 서버를 하나로 시작할 것인가? < O>
- Backend에서 MCP로 사용자 권한을 어떻게 전달할 것인가? < 사용자가 입력한 종목을 검색하고 요약. 조회 외의 권한 필요없음>
