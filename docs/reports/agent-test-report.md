...
| 09-07 `9dcc628` | 검증기 v3: 목표주가 비제시·직접 인용 문맥, 제한 동등 표현 인정, 상세 실패 보조 실측 | 오프라인 재검사 off 54/56, 실제 상세 호출 경로 off 400/on 완료 |

커밋 내용은 `git log`와 `git show`로 확인했습니다. 이력의 각 변경이 결과에 미친 영향을 이번 실험만으로 하나씩 검증한 것은 아닙니다. 현재 공시 매칭은 키워드 규칙이며 의미 유사도 모델이 아닙니다.
...
[Backend narrative_source 분기 시험](agent-test-result-report_narrative-source.md)은 성공 서술 채택·실패 시 규칙 조립을 담당합니다. 해당 문서는 수정하지 않았습니다. 그 문서의 live 환경 단일 표본 “조건부 PASS”를 재판정하지 않고, 미해결 다건 정량화를 이 보고서 6.4절에서 보완합니다.
