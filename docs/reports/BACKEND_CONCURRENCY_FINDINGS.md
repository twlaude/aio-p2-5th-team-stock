...
concurrency 300 기준, 같은 시점에서 sync와 나란히 비교하면 "풀 없는 async"는
sync보다 3.6~6.7배 느렸는데(위 표), "async + 커넥션 풀"은 반대로 sync보다
**1.7배 빠르다**. 20~100 구간은 두 방식의 동시 처리 한도(스레드풀 ~40 vs
커넥션 풀 20)가 비슷한 급이라 차이가 크지 않았다.

(참고로 맨 위 "결과" 표의 "~9x"는 이것과는 다른 축의 숫자다 — 그건 sync
혼자서 concurrency가 20→300으로 늘 때 자기 자신 기준으로 얼마나 느려지는지를
잰 것이고, 여기서 말하는 "1.7배"는 concurrency 300이라는 같은 지점에서 sync와
async+풀 "두 시스템"을 비교한 것이다. 두 숫자를 하나의 개선 전후처럼 이어
읽지 않도록 주의한다.)
...
sync 버전에 같은 1초 **블로킹** 지연을 넣어 직접 재현하지는 않았다(비교하려면
코드베이스 전체를 되돌려야 해서). 대신 스레드풀 크기(~40)로 계산하면, 1초씩
