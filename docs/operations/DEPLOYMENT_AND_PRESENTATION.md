...
- 일곱 서비스는 각각 독립 프로세스다. 한 컴퓨터에 모아 띄울 수도 있고, 팀원 컴퓨터마다 나눠 띄울 수도 있다. 주소와 포트는 `.env`로 연결한다.
- 발표 시연용으로는 서비스 일곱 개를 띄운 시연 서버를 두고 README 접속 주소로 들어간다. 시연 서버에서도 실행 방법은 로컬과 같다(각 폴더에서 `python server.py` / `uvicorn` / `npm run dev`).
