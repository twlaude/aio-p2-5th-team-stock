...
<a href="../architecture/diagrams/user-app-flow.svg"><picture><source media="(prefers-color-scheme: dark)" srcset="../architecture/diagrams/user-app-flow-dark.svg"><img src="../architecture/diagrams/user-app-flow.svg" alt="사용자 앱 화면 이동 흐름" width="100%"></picture></a>

[Mermaid 원본](../architecture/diagrams/user-app-flow.mmd)
...
첫 번째 카드는 `market_temperature.score`를 0~100으로 제한해 시장 관심 온도로 보여 줍니다. 온도는 거래량과 커뮤니티 글 수의 평소 대비 비율, 기간 내 관련 뉴스 건수, 공포탐욕 강도를 정규화해 합성합니다. 커뮤니티가 포함되면 "거래량 변화 · 뉴스 기사량 · 커뮤니티 글 수 · 공포탐욕 강도 기준. 이 종목의 평소 대비예요. 상승 가능성이 아니에요."라고 설명하고, 커뮤니티가 빠지면 "거래량 변화 · 뉴스 기사량 기준 (커뮤니티 제외)"를 보여 줍니다. `weight_covered < 100`이면 미가용 항목을 빼고 받은 자료만으로 재정규화한 결과이므로 "일부 자료를 못 받아 받은 자료만으로 계산했어요."를 한 줄 덧붙입니다. 가용 배점 숫자는 화면에 노출하지 않습니다.

두 번째 카드는 `evidence_level.level`을 낮음="아직 조금", 중간="절반쯤", 높음="많이 확인됨"의 3단계로 바꿔 보여 줍니다. 이 단계는 최근 30일 주요 비정기 공시가 현재 뉴스·커뮤니티 이슈와 직접 연결되는지를 뜻하며, 캡션은 "뉴스 내용이 공시·보고서로 뒷받침되는 정도"로 표시합니다. 연결 이슈는 `confirmed`, 미연결 이슈는 `unconfirmed` 패널에 보여 주고 공시 항목에는 주요 공시·정기 배지를 붙입니다.
...
부분 실패는 전체 분석을 빈 화면으로 만들지 않습니다. 예를 들어 커뮤니티가 빠지면 관심 온도 설명에서 커뮤니티를 제외하고 일부 자료만으로 계산했다는 문구를 한 줄 덧붙입니다. 커뮤니티 카드에는 제외 이유를 표시하며 뉴스·공시·가격은 계속 보여 줍니다.
